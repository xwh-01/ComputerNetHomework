...
def build_parser() -> argparse.ArgumentParser:
    ...
    encode_parser.add_argument(
        ...
        type=float,
        default=None,
        help="Maximum duration (in seconds) to encode, only encode this much data, default: unlimited",
    )
    encode_parser.add_argument(
        "--save-cut",
        action="store_true",
        help="Save the truncated binary data as *_cut.bin when --max-seconds is used",
    )
...
def main() -> int:
    ...
    if args.command == "encode":
        ...
        frame_count, byte_count, kbps = transport.encode_file_to_video(
            args.input_file,
            args.output_video,
            fps=args.fps,
        ...
        print(f"Encoded {byte_count} bytes at {kbps:.2f} kbps")
        return 0
