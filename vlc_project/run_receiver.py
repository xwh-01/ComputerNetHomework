...
class VLCReceiver:
    ...
    def detect_sync_frames(self, brightness_list: list[float]) -> tuple[int, int]:
        ...
        # 修复结尾同步帧检测：只在视频后半段检测（避免误判数据帧）
        # 核心：结尾同步帧只能出现在 总帧数 - sync_num*2 之后
        min_end_search_idx = len(brightness_list) - sync_num * 2
        if min_end_search_idx < start_idx:
            min_end_search_idx = start_idx + 10  # 至少在数据帧起始后10帧

        for i in range(min_end_search_idx, len(brightness_list) - sync_num + 1):
            window = brightness_list[i:i+sync_num]
